...
class ApplyAttendance(View):
    ...
    def get(self, request):
        ...
        if not daily_leave.exists():
            ...
            if daily_leave[0].date_is_half:
                ...
                in_time = request.user.user_basics.shift_details.in_time.strftime(
                    "%H:%M:%S")
                out_time = request.user.user_basics.shift_details.out_time.strftime(
                    "%H:%M:%S")
        ...
        month_full_day_present_count = DailyAttendance.objects.filter(
            emp_user_id=user_id, month=get_month_year(), is_present=True).count()
        month_half_day_present_count = DailyAttendance.objects.filter(
            emp_user_id=user_id, month=get_month_year(), is_present=True, is_half_day=True).count()
        ...
        months = {month: index for index,
                  month in enumerate(month_abbr) if month}
        ...
        month_full_day_leaves = DailyLeave.objects.filter(
            emp_user_id=user_id, date__gte=first_date, date__lte=last_date).count()
        month_half_day_leaves = DailyLeave.objects.filter(
            emp_user_id=user_id, date__gte=first_date, date__lte=last_date, date_is_half=True).count()
        if month_full_day_leaves == 0:
            month_leaves_count = Decimal(month_half_day_leaves*0.5)
        else:
            month_leaves_count = Decimal(
                month_full_day_leaves - (month_half_day_leaves * 0.5))

        holiday_month_count = Decimal(
            Holiday.objects.filter(month=get_month_year()).count())
        ...
        holiday_month_count_ab = Decimal(Holiday.objects.filter(
            date__gte=first_date, date__lte=today_date).count())

        month_full_day_leaves_ab = DailyLeave.objects.filter(
            emp_user_id=user_id, date__gte=first_date, date__lte=today_date).count()
        month_half_day_leaves_ab = DailyLeave.objects.filter(
            emp_user_id=user_id, date__gte=first_date, date__lte=today_date, date_is_half=True).count()
        if month_full_day_leaves_ab == 0:
            month_leaves_count_ab = Decimal(month_half_day_leaves_ab*0.5)
        else:
            month_leaves_count_ab = Decimal(
                month_full_day_leaves_ab - (month_half_day_leaves_ab * 0.5))

        month_full_day_present_count_ab = DailyAttendance.objects.filter(
            emp_user_id=user_id, date__gte=first_date, date__lte=last_date, is_present=True).count()
        month_half_day_present_count_ab = DailyAttendance.objects.filter(
            emp_user_id=user_id, date__gte=first_date, date__lte=last_date, is_present=True, is_half_day=True).count()
        if month_full_day_present_count_ab == 0:
            month_presenty_count_ab = Decimal(
                month_half_day_present_count_ab*0.5)
        else:
            month_presenty_count_ab = Decimal(
                month_full_day_present_count_ab - (month_half_day_present_count_ab*0.5))

        absent_days = Decimal(datetime.now().strftime('%d')) - holiday_month_count_ab - \
            month_leaves_count_ab - month_presenty_count_ab - weekdays_ab
    ...
    def post(self, request):
        ...
        if user_id == int(data['user_id']):
            ...
            daily_leave = DailyLeave.objects.filter(
                emp_user_id=user_id, date=date)
            holiday = Holiday.objects.filter(date=date).exists()
            ...
            if can_mark_attendance:
                ...
                if not daily_attendance.exists():
                    ...
                    if is_half_day:
                        ...
                        if is_first_half:
                            ...
                            if current_time > second_half_in_time_time + timedelta(hours=2):
                                messages.warning(
                                    request, 'your are too late can not mark todays attendance')
                    ...
                    DailyAttendance.objects.create(emp_user_id=user_id, date=date, month=month, login_time=current_time,
                                                   is_half_day=half_day, is_first_half=first_half, is_second_half=second_half, is_requested=True)
                    messages.success(
                        request, "your daily attendance processed successfully")
                    ...
                    if daily_attendance[0].logout_time is None:
                        ...
                        messages.success(
                            request, "your Out time updated successfully")

                    else:

                        messages.warning(
                            request, 'your request has already processed')

            else:
                messages.warning(
                    request, "you are on the leave thats why your attendance not mark")

        else:
            messages.success(
                request, "Your todays attendance request added successfully")
...
class MissedAttendance(View):
    ...
    def post(self, request):
        ...
        if request_date >= date or request_date < pre_date:
            messages.warning(
                request, 'You can\'t apply missing attendace for this date')
            return redirect('self_attendance')
        ...
        missing_attendance = MissingAttendance.objects.filter(
            emp_user_id=user_id, date=data['date'])
        ...
        if half_day:
            MissingAttendance.objects.create(
                emp_user_id=user_id, date=data['date'], is_half_day=half_day, is_first_half=first_half, is_second_half=second_half, is_requested=True)
            messages.success(request, 'added successfully')
            return redirect('self_attendance')

        else:
            MissingAttendance.objects.create(
                emp_user_id=user_id, date=data['date'], is_half_day=False, is_requested=True)
            messages.success(request, 'added successfully')
            return redirect('self_attendance')
    ...
    def get(self, request):
        ...
        for date in date_range:
            ...
            if Holiday.objects.filter(date=date).exists():
                ...
                leave = DailyLeave.objects.filter(
                    emp_user_id=user_id, date=date)
                attendance = DailyAttendance.objects.filter(
                    emp_user_id=user_id, date=date, is_present=True)
...
class MissingAttendanceHistory(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        missing_attendance_history = MissingAttendance.objects.filter(
            emp_user_id=user_id)
